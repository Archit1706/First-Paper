...
const PaperDetails = () => {
  ...
  let paperData = fetchedPapers.find((p) => p.url === `/${paper}`);
  // let paperData = fetchedPapers[0]
  if (!paperData) paperData = fetchedPapers[0];

  let abstractLink = "https://arxiv.org/abs/" + paperData.id;
  let paperLink = paperData.doi ? ("https://sci-hub.se/" + paperData.doi) : ("https://arxiv.org/pdf/" + paperData.id + ".pdf");
  return (
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-10 mx-auto">
        ...
        <div className="flex flex-row justify-evenly items-center">
          ...
          <button className="flex mx-auto mt-16 text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg">Absract</button>
          </a>
        <a href={paperLink} target="_blank">
          <button className="flex mx-auto mt-16 text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg">Paper</button>
      </a>
        </div>
      </div>
      <Link className="" href="/#papers">
        <p className="pl-10 pb-4 underline cursor-pointer">Back</p>
      </Link>
    </section>
  )
}
